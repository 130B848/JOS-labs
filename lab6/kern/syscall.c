...
static int
sys_exec(struct Trapframe *tf, struct Proghdr *ph, uint32_t phnum)
{
    //cprintf("syscall esp = 0x%x, eip = 0x%x, ph = 0x%x, phnum = %u\n", tf->tf_esp, tf->tf_eip, (uint32_t)ph, phnum);
    int i, perm, r;
    struct Page *pg;

    curenv->env_tf = *tf;

    uint32_t middleware = (uint32_t)UTEMP3;
    for (i = 0; i < phnum; i++, ph++) {
        ...
        uint32_t iter = ROUNDDOWN(ph->p_va, PGSIZE);
        for (; iter < ROUNDUP(ph->p_va + ph->p_memsz, PGSIZE); iter += PGSIZE, middleware += PGSIZE) {
            if ((pg = page_lookup(curenv->env_pgdir, (void *)middleware, NULL)) == NULL) {
                return -E_NO_MEM;
            }
            if ((r = page_insert(curenv->env_pgdir, pg, (void *)iter, perm)) < 0) {
                return r;
            }
            page_remove(curenv->env_pgdir, (void *)middleware);
        }
    }
...
static int
sys_pkt_transmit(uint8_t *data, uint32_t length) 
{
	user_mem_assert(curenv, (void *)data, length, PTE_U);
    return e1000_transmit(data, length);    
}

static int
sys_pkt_receive(uint8_t *data) 
{
	user_mem_assert(curenv, (void *)data, 2048, PTE_U);
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
		...
		case SYS_exec:
			return sys_exec((struct Trapframe *)a1, (struct Proghdr *)a2, a3);
        case SYS_pkt_transmit:
            return sys_pkt_transmit((uint8_t *)a1, a2);
        case SYS_pkt_receive:
            return sys_pkt_receive((uint8_t *)a1);
		default:
			return -E_INVAL;
	}
