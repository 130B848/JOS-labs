...
static int backup_map_segment(envid_t child, uintptr_t va, size_t memsz,
		       int fd, size_t filesz, off_t fileoffset, int perm);
...
exec(const char *prog, const char **argv)
{
	...
	for (i = 0; i < elf->e_phnum; i++, ph++) {
		...
		if (ph->p_flags & ELF_PROG_FLAG_WRITE)
			perm |= PTE_W;
		if ((r = backup_map_segment(this_id, middleware + PGOFF(ph->p_va), ph->p_memsz,
				     fd, ph->p_filesz, ph->p_offset, perm)) < 0)
			goto error;
    ...
    //cprintf("esp = 0x%x, eip = 0x%x\n", this_tf.tf_esp, this_tf.tf_eip);
	ph = (struct Proghdr*) (elf_buf + elf->e_phoff);
...
static int
backup_map_segment(envid_t this_id, uintptr_t va, size_t memsz,
	int fd, size_t filesz, off_t fileoffset, int perm)
{
	int i, r;
	void *blk;
