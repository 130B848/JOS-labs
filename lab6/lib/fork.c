...
pgfault(struct UTrapframe *utf)
{
	...
	if (!(err & FEC_WR)) {
        panic("Faulting access is not a write.");
    }
	if (!(vpd[PDX(addr)] & PTE_P)) {
		panic("Faulting access: page dir is not present.");
    }
	if (!(vpt[PGNUM(addr)] & PTE_P) || !(vpt[PGNUM(addr)] & PTE_COW)) {
		panic("Faulting access is not to COW page.");
	}
	...
	if (r) {
		panic("pgfault unmap temporary page failed %e", r);
	}
...
sfork(void)
{
    // If you want to run sfork, please comment the "panic" and change O1 to O0
	...
	if (child_id < 0) {
		...
		for (pn = UTEXT / PGSIZE; pn < (UTOP - PGSIZE) / PGSIZE; pn++) {
			if ((vpd[pn / NPTENTRIES] & PTE_P) &&
					(vpt[pn] & PTE_P) && (vpt[pn] & PTE_U)) {
                if (pn == (USTACKTOP - PGSIZE) / PGSIZE) {
				    duppage(child_id, pn);
                } else {
				    sharepage(child_id, pn);
                }
			}
        }
