...
send_data(struct http_request *req, int fd)
{
    ...
    if ((r = fstat(fd, &sb)) < 0) {
        die("send_data failed: fstat error");
    }

    if (sb.st_size > MAX_PKT_SIZE) {
        die("send_data failed: size > 1518");
    }

    if ((r = readn(fd, buf, sb.st_size)) != sb.st_size) {
        die("send_data failed: read error");
    }

    if ((r = write(req->sock, buf, sb.st_size)) != sb.st_size) {
        die("send_data failed: write error");
    }
