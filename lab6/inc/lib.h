...
//extern const volatile struct Env* _thisenv();
//#define thisenv (_thisenv())
...
int sys_exec(struct Trapframe *tf, struct Proghdr *ph, uint32_t phnum);
int sys_pkt_transmit(uint8_t *data, uint32_t length);
int sys_pkt_receive(uint8_t *data);
